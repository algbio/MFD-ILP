...
def decompose_flow(vertices, fdown, fup, out_neighbors, in_neighbors, source, sink, max_flow, K, threads, weighttype):
    ...
    try:
        ...
        # flow explanation inexact
        for (i,j) in E:
            model.addConstr(fup[(i,j)] >= sum(z[i,j,k] for k in range(0,K)))
            model.addConstr(fdown[(i,j)] <= sum(z[i,j,k] for k in range(0,K)))
...
graph = read_input_inexact(args.input, args.weighttype)
fdown = graph['fdown']
fup = graph['fup']
...
for K in range(minK, maxK + 1):
    data = decompose_flow(vertices, fup, fdown, out_neighbors, in_neighbors, source, sink, max_flow, K, threads, args.weighttype)
